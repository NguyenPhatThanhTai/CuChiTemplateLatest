...
(function ($) {
                ...
                <tbody>
                  ${rows
                    ...
                    <tr data-id="${r.id}" data-ids="${(r.image_ids || []).join(",")}">
                      ...
                      <td>${r.total_rooms}</td>
                      <td>
                        <button class="btn btn-link btn-images">View / Change (${(r.image_ids || []).length})</button>
                      </td>
                    </tr>`
                    )
                    .join("")}
    ...
    async function openCreateRoomModal({ onSaved }) {
      ...
      body.innerHTML = `
        <div class="form two-col">
          <label>Name<input id="cr_name" type="text" required></label>
          <label>Type<input id="cr_type" type="text"></label>
          <label>Slot<input id="cr_slot" type="number" min="1" value="1"></label>
          <label>Total rooms<input id="cr_total" type="number" min="1" value="1"></label>
          <label>Price<input id="cr_price" type="number" step="0.01" min="0" value="0"></label>
        </div>
    ...
    async function pickOneFromMedia() {
      return new Promise((resolve) => {
        ...
        frame.on("select", () => {
          const a = frame.state().get("selection").first();
          const id = a ? a.get("id") : null;
          resolve(id || null);
        });
